			...
			if (event.target.id === "newest") {
				if (filterByFilter !== null) {
					const { data } = await supabase
						.from("posts")
						.select(
							"id, created_at, title, upvotes, user_id, flair"
						)
						.eq(
							"flair",
							filterByFilter.charAt(0).toUpperCase() +
								filterByFilter.slice(1)
						)
						.order("created_at", { ascending: false });
					setPosts(data);
				} else {
		...
		<div>
			<div className="sort-filter">
				<div className="sort-container">
					...
					<h4
						...
						className={
							sortByFilter === "newest"
								? `sort-flair ${theme}-bg active`
								: `sort-flair ${theme}-bg ${theme}-bg-hover`
						}
						onClick={handleSort}
					>
						Newest
			...
			{loading ? (
				...
				<div className="posts-container">
					{posts.map((post, i) => {
						return <PostCondensed key={i} post={post} />;
					})}
				</div>
			) : (
				<div className="no-posts">
					...
					<p>
						Get started on contributing by{" "}
						<Link className={`${theme} create-link`} to="/create">
							creating a new post
						</Link>
						!
					</p>
