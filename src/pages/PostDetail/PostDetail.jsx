	...
	useEffect(() => {
		const fetchPostInfo = async () => {
			const post = await supabase
				.from("posts")
				.select(
					"id, created_at, title, content, url, upvotes, user_id, video, flair, uploaded"
				)
				.eq("id", id)
				.single();
	...
	const handleSubmit = async (event) => {
		if (event.key === "Enter") {
			if (userComment.replace(/\s/g, "") !== "") {
				setComments([
					...comments,
					{ user_id: UID, comment: userComment },
				]);
				await supabase
					.from("comments")
				...
				<div className="post-detail">
					...
					<div className="post-btns">
						<div className="upvote-container">
							<Icon
								className={`${theme} upvotes-icon`}
								icon={"bxs:upvote"}
								width={"1.5rem"}
								height={"1.5rem"}
								onClick={handleUpvote}
							></Icon>
							<p className="post-detail-upvotes">
						...
						{deleteLoading ? (
							...
							<>
								...
								<DialogContent dividers>
									...
									>
										The post was successfully deleted. Click
										on the button below to return to the
										home page.
									</DialogContentText>
								</DialogContent>
								<DialogActions className="delete-dialog-actions">
									<div className="delete-dialog-btns-container">
										<div
											className={`${theme}-bg delete-dialog-btns`}
											onClick={() => navigate("/")}
										>
											Back to Home
										</div>
							...
							<>
								...
								<DialogContent dividers>
									<DialogContentText
										...
										className="dialog-content"
									>
										There was an issue with deleting the
							...
							<>
								...
								<DialogContent>
									<DialogContentText
										id="alert-dialog-description"
										className="secret-key-content"
									>
										This process cannot be undone. Please
										enter the post's secret key to confirm.
									</DialogContentText>
									<TextField
										FormHelperTextProps={{
											style: {
										...
										helperText={
											secretKeyErr
												? "The secret key inputted is incorrect. Please try again."
												: ""
										}
									/>
								</DialogContent>
								<DialogActions className="dialog-actions">
									<div className="dialog-btns-container">
										<div
											className={`${theme}-bg secret-key-dialog-btns ${theme}-bg-hover`}
											onClick={handleCloseDialog}
										>
											Cancel
										</div>
