			...
			if (urlType === "upload") {
				if (uploadFile === null) {
					...
					if (error) {
						setError(true);
						setMessage(
							"There was an error with updating the post. Please try again."
						);
						setLoading(false);
					} else {
						setPostID(data.id);
			...
			{secretKeyValidated ? (
				<div className="update-post-form">
					<Box className="form-container" component="form">
								...
								<TabPanel value="2" sx={{ p: 1.5 }}>
									...
									uploadURL.replace(/\s/g, "") === "" ? (
										<p className="no-url-message">
											There is nothing to be previewed
											currently. Please input a url or
											upload an image in the previous tab.
										</p>
									) : urlType === "image" ? (
										<img
											src={postURL}
						...
						<button
							className={`${theme}-bg update-post-btn ${theme}-bg-hover`}
							type="submit"
								...
								<>
									...
									<DialogContent dividers>
										<DialogContentText
											...
											className="dialog-content"
										>
											Post was updated successfully! View
											the updated post or return home.
										</DialogContentText>
									</DialogContent>
					...
					{secretKeyLoading ? (
						...
						<>
							...
							<DialogContent>
								...
								>
									In order to update the post, please enter
									the secret key associated with the post.
								</DialogContentText>
								<TextField
									FormHelperTextProps={{
										style: {
